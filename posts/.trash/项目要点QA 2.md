...
**CallerRunsPolicy**，让提交任务的线程自己来执行。

**AbortPolicy**, 直接抛异常不干了。

**DiscardPolicy**, 不做任何处理，悄悄丢掉任务。
...
2. 异步通知：异步通知消息的，就是如果有人点赞了异步通知
3. 异步更新去：除了缓存写的时候，还有就是在于更新视频到OSS上
...
JDK21出现的虚拟线程解决的是IO密集型任务，相当于给平台线程套了一个壳子：
至于谁来调度虚拟线程，JVM来：
没有虚拟线程：线程阻塞只能挂起少线程，或者等着
有虚拟线程：虚拟线程跑在平台线程上，进入IO等待，虚拟线程去挂起，平台线程空出来了

==这个项目的场景更多是IO密集型，比如等待拉取聊天记录，所以2倍CPU核心数的**核心线程**，**最大线程**是核心线程2倍，**阻塞队列**是`LinkedBlockingQueue`并且设置最大容量是512，**拒绝策略**是`CallerRunsPolicy`==

#### 任务失败或报错怎么办

异常处理`.handle((result, ex) -> { ... })`抓异常就ok
#### QPS爆升怎么办

因为配置的拒绝策略是`CallerRunsPolicy`，主线程接手进行任务，可能会变慢，但是不会接口宕机。
## Netty&WebSocket
...
7. 推送数据：
   ...
   8. 获取发送者自己的Channel集合（多端同步）
   9. 获取接收者所有的Channel集合（消息转发）
   10. 出站：推送消息到对应的EventLoop上，启动出站Pipeline
...
4. （业务）开始上传与转码
   ...
   6. （后→mysql）把原始`MP4URL`和`status`(*准备转码*)存入数据库
   7. （后→Kafka）向Kafka发送转码任务
8. （后→redis）存储视频信息，视频状态，统计数据
9. （Kakfa→OSS）执行FFmpeg转码，并把HLS文件上传到OSS
10. （最终）监听Kafka接受成功通知，更新最终url以及状态
...
3. 播放器拿到url
   4. HLS： 播放器请求 M3U8，开始 **ABR 自适应码率**播放。
   5. MP4：播放器直接加载原始 MP4 文件
...
3. 可以引入存储清理策略
   4. 可以把**高成本的 OSS 存储**（例如**标准存储**）转移到**低成本的**归档存储（如 OSS **低频访问型或归档型**）。
   5. 清理原url。
   6. 最终剩下归档的备份原始文件以及HLS文件。
...
### 选型
| **维度**     | **iText 8**              | **Apache PDFBox**  | **OpenPDF**  |
| ---------- | ------------------------ | ------------------ | ------------ |
| **开发抽象层级** | **高**（像写 HTML/CSS，有布局引擎） | **低**（像画图，需手动计算坐标） | **中**（功能较单一） |
| **复杂排版**   | 完美支持多级表格、嵌套布局            | 实现起来极其痛苦，代码量巨大     | 仅能处理基础布局     |
| **中文字体**   | 极其友好（font-asian 模块）      | 需要自行处理复杂的字体加载逻辑    | 较老旧，兼容性一般    |
...
本项目导出 PDF 的第一版采用的就是非内嵌字体方案，结果遇到使用 Linux 系统的朋友出现导出乱码

## RedisStream

### 选型

Redis Stream 是 Redis 5.0 引入的数据结构，专为消息队列场景设计。对于我的项目而言：  
1. 复用现有的组件：项目已经使用 Redis 做缓存，无需额外部署消息队列。 
2. 消息确认机制：通过 ACK 机制确保消息不丢失。
3. 轻量级：相比 Kafka/RabbitMQ，运维成本更低。

### 流程

![image.png](https://image-yupan.cixingji.cloud/study/20260116153749629.png)
#### 流程
![image.png](https://image-yupan.cixingji.cloud/study/20260116155729594.png)

### 重试机制

#### 自动重试
![image.png](https://image-yupan.cixingji.cloud/study/20260116155424517.png)
#### 手动重试
...
**SSE (Server-Sent Events)** 是一种在 HTTP 协议之上的长连接技术。

- **握手阶段**：客户端发送一个标准的 HTTP 请求，但在 Header 中明确要求接收事件流：`Accept: text/event-stream`。
    
- **保持连接**：服务端不关闭连接，而是将响应头设置为 `Content-Type: text/event-stream`，并保持 TCP 通道开启。
    
- **数据传输**：服务端按固定格式不断推送数据片段
### AI输出用它的优点
无需连续的重复握手
#### 1.流式输出

大语言模型（LLM）生成回答是逐个 Token（字/词）产出的。

- **普通 HTTP**：必须等待模型生成完几百个字，后端一次性返回。用户会盯着加载圈看 10 秒钟，体验极差。
    
- **SSE 模式**：模型每生产一个字，后端就顺着“水管”推一个字。用户能立刻看到“打字机”效果。
...
虽然 WebSocket 也能实现流式，但在 AI 问答场景下它属于“过度设计”：

- **单向性匹配**：AI 问答是“一问多答”。用户发一个 Prompt（请求），后续全是服务器在“吐”数据。这种**单工/半双工**需求，SSE（单向推送）完美契合。
    
- **有状态 vs 无状态**：
    
    - **WebSocket** 是有状态的，每个连接都要占用服务器内存，且在微服务集群中要做“连接共享”或“粘性会话”，非常麻烦。
        
    - **SSE** 本质上是 **HTTP**，它是无状态的。Nginx、网关（Spring Cloud Gateway）对 HTTP 的负载均衡支持非常成熟，不需要额外配置。
        
- **自动重连**：SSE 协议内置了 `retry` 机制。如果网络闪断，浏览器会自动重新连接并尝试恢复流，WebSocket 则需要你自己写复杂的重连逻辑。