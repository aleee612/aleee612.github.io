...
---
**主要思路：**
1. 用Math.random()随机生成范围为`[1,v]`的数，存放在长度为N的数组中
2. 这样的操作执行testTime次，若测试得出暴力算法与优化算法结果不同则输出测试用例
 ```java title=对数器 fold
package org.example;  
...
public class Main {  
    ...
    public static void main(String[] args) {  
        ...
        int V = 1000;  
        // testTimes : 测试次数  
        int testTimes = 50000;//相当于很多题目中的while(t--)  
        System.out.println("测试开始");  
        for (int i = 0; i < testTimes; i++) {  
            // 随机得到一个长度，长度在[0~N-1]  
            int n = (int) (Math.random() * N);//Math.random()：随机生成一个[0,1)之间的double  
            // 得到随机数组  
            int[] arr = randomArray(n, V);  
            int[] arr1 = copyArray(arr);//这是由于java引用对象的特性  
